...
if [ -n "$(git status --porcelain)" ]; then
    echo -e "\e[0;31m BUILD FAILED!\e[0m\n\nYou Have uncommited changes.\n Commit them to proceed[0m";
    exit 0;
else
  echo -e "\e[0;34mdeploy :: ----------------> No Changes found [0m";

#enter the user account directory
echo -e "\e[0;34mdeploy :: ----------------> Preparing To Deploy[0m"
cd pwa
...
#build static files
npm run build
echo -e "\e[0;34mdeploy :: ----------------> Static Files Successfully Built[0m"
...
echo -e "\e[0;34mdeploy :: ----------------> Copying Pwa Static Files to App Directory[0m"
mkdir ./../src/views/pages/account
cp -r ./pwa/* ./../src/views/pages/account

echo -e "\e[0;34mdeploy :: ----------------> Static Files Copied to App Directory[0m"

#exit the pwa direcory
cd ..
echo -e "\e[0;34mdeploy :: ----------------> In Root Directory ...[0m"

#add the changes to commit 
echo -e "\e[0;34mdeploy :: ----------------> Adding Changes to Git Commit[0m"
git add .
git commit -m "CHORE -> added pwa static files to the app directory[0m"
git push 
echo -e "\e[0;34mdeploy :: ----------------> Changes successfully pushed to repository\nDeploying application[0m"
